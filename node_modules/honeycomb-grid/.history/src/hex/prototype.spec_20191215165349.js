...
describe.only('nudge', function() {
  it('returns the current hex with a tiny offset on each cube coordinate', function() {
    ...
    expect(hex).not.to.contain(result)
    expect(result.q).to.be.closeTo(1, 0.000001)
    expect(result.r).to.be.closeTo(-3, 0.000001)
    expect(result.s).to.be.closeTo(2, 0.000001)
    // const hex = Hex({ q: 1, r: -3, s: 2 })
