...
export function ringFactory({ Grid, Hex }) {
  return function ring({ center, radius }) {
    const grid = new Grid()
    // const offset = offsetFromZero(center.offset, center.q)
    // let currentHex = center.add(Hex({ q: 0, r: -radius - offset, s: radius + offset }))
    // let currentHex = center.add(
    //   Hex(-radius + (radius % 2 === 0 || center.r !== 0 ? 1 : Math.round(radius / 2)), -radius),
    // )
    // let currentHex = center.add(Hex(-radius + (radius % 2 === 1 && center.r !== 0 ? 2 : 1), -radius))
    let currentHex = Hex({ q: 0, r: center.r - radius, s: center.s + radius })

    // for (let i = 0; i < 6; i++) {
    //   for (let j = 0; j < radius; j++) {
    grid.push(currentHex)
    //     const { q, r, s } = DIRECTION_COORDINATES[i]
    //     currentHex = Hex({ q: currentHex.q + q, r: currentHex.r + r, s: currentHex.s + s })
    //   }
    // }
...
// export function spiralFactory() {
//   return function spiral({ radius, center, onCreate = () => {} }) {
//     let hexes = [center]

//     for (let i = 1; i <= radius; i++) {
//       hexes = hexes.concat(ring(center, i))
//     }
