...
export function hexesInRangeFactory({ isValidHex }) {
  ...
  return function hexesInRange(centerHex, range = 0, includeCenterHex = true) {
    ...
    if (!this.get(centerHex)) {
      throw new Error(
        `Center hex with coordinates ${centerHex} not present in grid.`
      )
    }
    ...
    for (let q = -range; q <= range; q++) {
      for (
        ...
        const hex = this.get(
          centerHex.cubeToCartesian({ q: centerHex.q + q, r: centerHex.r + r })
        )
...
export function neighborsOfFactory({
  isValidHex,
  signedModulo,
...
export function pointWidth() {
  const { isPointy, width } = this[0]
  const { 0: first, length, [length - 1]: last } = isPointy()
    ? [...this].sort((a, b) => b.s - a.s || a.q - b.q)
    : [...this].sort((a, b) => a.q - b.q)

  return last.toPoint().x - first.toPoint().x + width()
}

export function pointHeight() {
  const { isPointy, height } = this[0]
  const { 0: first, length, [length - 1]: last } = isPointy()
    ? [...this].sort((a, b) => a.r - b.r)
    : [...this].sort((a, b) => b.s - a.s || a.r - b.r)

  return last.toPoint().y - first.toPoint().y + height()
}
