...
describe('hexesBetween', () => {
  ...
  it('calls firstHex.nudge(), firstHex.lerp(), lastHex.nudge() and firstHex.round() for each hex between firstHex and lastHex', () => {
    ...
    expect(lerp).to.always.have.been.calledWith(
      'last hex nudge result',
      sinon.match.number
    )
...
describe('hexesInRange', () => {
  ...
  it(`throws when a hex is passed that isn't present in the grid`, () => {
    ...
    expect(() => hexesInRange(Hex(1, 2))).to.throw(
      `Center hex with coordinates 1,2 not present in grid.`
    )
  })
...
describe('neighborsOf', () => {
  ...
  describe('when called with compass direction(s)', () => {
    it('calls compassToNumberDirection', () => {
      ...
      expect(compassToNumberDirectionSpy).to.have.been.calledWith(
        'NW',
        'pointy'
      )
    })
...
describe('pointWidth', () => {
  ...
  describe('when the grid contains pointy hexes', () => {
    ...
    it('returns the total width (in points/pixels) of the grid', () => {
      expect(
        GridFactory.rectangle({ ...options, direction: 0 }).pointWidth()
      ).to.be.closeTo(155.8846, 0.0005)
      expect(
        GridFactory.rectangle({ ...options, direction: 1 }).pointWidth()
      ).to.be.closeTo(121.2436, 0.0005)
      expect(
        GridFactory.rectangle({ ...options, direction: 2 }).pointWidth()
      ).to.be.closeTo(121.2436, 0.0005)
      expect(
        GridFactory.rectangle({ ...options, direction: 3 }).pointWidth()
      ).to.be.closeTo(155.8846, 0.0005)
      expect(
        GridFactory.rectangle({ ...options, direction: 4 }).pointWidth()
      ).to.be.closeTo(103.923, 0.0005)
      expect(
        GridFactory.rectangle({ ...options, direction: 5 }).pointWidth()
      ).to.be.closeTo(103.923, 0.0005)
    })
  })
...
describe('pointHeight', () => {
  ...
  describe('when the grid contains pointy hexes', () => {
    ...
    it('returns the total width (in points/pixels) of the grid', () => {
      expect(
        GridFactory.rectangle({ ...options, direction: 0 }).pointHeight()
      ).to.equal(70)
      expect(
        GridFactory.rectangle({ ...options, direction: 1 }).pointHeight()
      ).to.equal(130)
      expect(
        GridFactory.rectangle({ ...options, direction: 2 }).pointHeight()
      ).to.equal(130)
      expect(
        GridFactory.rectangle({ ...options, direction: 3 }).pointHeight()
      ).to.equal(70)
      expect(
        GridFactory.rectangle({ ...options, direction: 4 }).pointHeight()
      ).to.equal(160)
      expect(
        GridFactory.rectangle({ ...options, direction: 5 }).pointHeight()
      ).to.equal(160)
    })
  })
  ...
  describe('when the grid contains flat hexes', () => {
    ...
    it('returns the total width (in points/pixels) of the grid', () => {
      expect(
        GridFactory.rectangle({ ...options, direction: 0 }).pointHeight()
      ).to.be.closeTo(138.564, 0.0005)
      expect(
        GridFactory.rectangle({ ...options, direction: 1 }).pointHeight()
      ).to.be.closeTo(86.6025, 0.0005)
      expect(
        GridFactory.rectangle({ ...options, direction: 2 }).pointHeight()
      ).to.be.closeTo(121.2436, 0.0005)
      expect(
