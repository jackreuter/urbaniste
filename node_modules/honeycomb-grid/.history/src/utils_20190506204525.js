...
export function ensureWidthAndHeight(size, isPointy) {
    if (isObject(size)) {
        const { xRadius, yRadius, width, height } = size

        if (isNumber(xRadius) && isNumber(yRadius)) {
            return size
        }
    ...
    if (isNumber(size)) {
        return isPointy
            ? { xRadius: size / Math.sqrt(3) * Math.sqrt(3) / 2, yRadius: size / 2 }
            : { xRadius: size / 2, yRadius: size / Math.sqrt(3) }
    }
